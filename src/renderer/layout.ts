...
export function layoutDiagram(diagram: Diagram, portMeta: PortMeta[] = [], options?: RenderOptions): LayoutResult {
  ...
  if (findWireCrossings(heuristic.wires, heuristic.junctions).length === 0) return heuristic;
  ...
  if (cc !== ch) return cc < ch ? crossmin : heuristic;
  if (bends(crossmin) !== bends(heuristic)) return bends(crossmin) < bends(heuristic) ? crossmin : heuristic;
  return crossmin.height < heuristic.height ? crossmin : heuristic;
}
