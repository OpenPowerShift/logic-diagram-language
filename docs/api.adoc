...
- The label/id toggles live on `options`: `showLabels` (default `true`, draws `.Name`/`.Description`
  labels) and `showIds` (default `false`, draws bare identifiers like `I1`, `AND#G1`). They are view
  toggles, so they are not authored via `OPTION` lines — set them in code: `{ ...options, showIds: true }`.
...
writeFileSync('diagram.svg', renderDiagram(diagram, resolveOptions(diagram.options)));
----
