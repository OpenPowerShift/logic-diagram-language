...
| `OUTPUT_ORDER` | `DECLARATION` (default), `AUTO`
| Stack outputs in declared order, or reorder them by their driver to reduce crossings.
...
| `COLUMN_SPACING` | `UNIFORM` (default), `ADAPTIVE`
| Fixed column pitch, or size each gap to its content for a narrower diagram.
