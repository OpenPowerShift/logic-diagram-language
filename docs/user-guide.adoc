...
| `LABEL_STYLE` | `BELOW` (default), `SIDE`
| Where a gate/function-block `.Description` is drawn. `BELOW` stacks it under the body;
`SIDE` places it in the upper-right beside the body, clear of the routing channel beneath
(best when there is open space to the block's right).
