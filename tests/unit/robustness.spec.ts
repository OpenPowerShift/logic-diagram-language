...
const CORPUS: Record<string, string> = {
  ...
  fbsealin: `OPTION OUTPUT_ORDER = AUTO\nDAN = FB#DAN(IN=ISET, TRIG=DAN, SEAL=ZZ).TRIP`,
  fbsealin2: `OPTION OUTPUT_ORDER = AUTO\nDAN = FB#DAN(TRIG=DAN, IN=A, SEAL=B, X=C).TRIP`,
  fbsealin3: `OPTION OUTPUT_ORDER = AUTO\nDAN = FB#DAN(IN=A, TRIG=B, SEAL=DAN, X=C, Y=D).TRIP`,
  fbsealincmp: `OPTION COMPACTNESS = COMPACT\nDAN = FB#DAN(TRIG=DAN, IN=A, SEAL=B, X=C, Y=D, W=E).TRIP`,
  srsealin: `OPTION OUTPUT_ORDER = AUTO\nQ = SR#L(SET OR (Q AND NOT B), RST).Q`,
  timersealin: `T = TIMER#TD(SR#L(START OR (T AND NOT B), RST).Q, 0, 5cyc)`,
  label: `G = A OR B OR C\nO = G AND D\nG.Name="OC"\nG.Description="51"`,
