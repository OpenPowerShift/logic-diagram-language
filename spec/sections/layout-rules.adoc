...
. Protected zone (enforce a minimum vertical gap between adjacent gate bodies in a column, so gates that share an input are not jammed together; the small offset this adds to a pushed gate's wires is kept >= `MIN_DOGLEG` so they stay clean Z-routes)
...
. The wire enters the destination port horizontally. For AND/NOT inputs it enters at the
  ...
  (see <<or-curve-ports>>).
. **Minimum gate entrance.** When a wire turns into a gate input port, its final horizontal
  run (the entrance) MUST be at least `GATE_ENTRANCE` (15px) long, so the vertical turn sits
  clear of the gate body/curve and the wire visibly enters horizontally rather than turning on
  the gate's edge. Equivalently, a turning wire's vertical channel sits at least `GATE_ENTRANCE`
  to the left of the port's X. This matters most for OR inputs, whose curve-tap X lies inside
  the bounding box: without the rule the channel would hug (or fall on) the box's left edge.
...
* **Validated** — a channel is only used if it clears all gate bodies and does not overlap
  a wire from a different net; a group falls back to its balanced-Z route if any channel in
  it cannot be placed cleanly, so the optimisation never introduces a new collision.
...
renderer generates more than one candidate input ORDERING and keeps the one that **renders the fewest
actual (geometric) crossings** — because the classic combinatorial layer-crossing count diverges
from the crossings actually drawn once gate heights, multi-port entry and routing doglegs are taken
into account. Candidates currently are: `heuristic` (barycentre + contiguity ordering — always a
candidate, so the result can never be worse than it) and `crossmin` (a Sugiyama median/transpose
optimiser on a dummy-expanded layered graph, port-aware so fixed-port blocks stay uncrossed). The
`crossmin` candidate is only computed when `heuristic` is not already crossing-free.
