...
. **Junction detection**, a final uniform vertical re-normalisation (which shifts every node, wire,
  ...
  **blank-band collapse**: any horizontal band occupied by no node body, label, or wire *feature* — a
  wire feature being a vertex (turn/endpoint) or a horizontal run, but NOT the interior of a vertical
  run merely passing through — is reduced to a fixed section gap, removing wasted vertical space. This
  covers both fully-disconnected sections and *weakly-connected* ones that meet only through the band
  (e.g. an SR-latch cluster and an analog cluster joined solely at a final gate, the latch's Q wire
  dropping through the gap). Being a uniform per-section shift, it preserves every wire shape — the
  threaded verticals stay straight, just shorter — and cannot cause an overlap or crossing.
