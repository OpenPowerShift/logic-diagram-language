...
. **Wire routing** (obstacle-aware A* fan-out), then the reshaping passes: channel-track assignment,
  ...
  (<<output-placement>>), sub-`MIN_DOGLEG` straightening, feedback routing, shared fan-out trunk
  merge, and the gate-entrance guarantee (<<gate-entrance>>).
. **Junction detection** and a final uniform vertical re-normalisation, which shifts every node, wire,
  junction AND label together so labels never drift from the gates they annotate.
